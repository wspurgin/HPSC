...
double& Mat::operator()(long int i, long int j) {
  ...
  if (i<0 || i>=nrows || j<0 || j>=ncols) {
    ...
    cerr << "  returning first entry to avoid seg. fault\n";
    return data[0];
  }
...
int Mat::Axpy(const Mat &A, const Mat &x) {
  ...
  if ((nrows != A.nrows) || (A.ncols != x.nrows) || (ncols != 1) || (x.ncols != 1)) {
    cerr << "Mat::Axpy error, illegal matrix dimensions\n";
    cerr << "  A is " << A.nrows << " x " << A.ncols << ",  x is " << x.nrows 
	 << " x " << x.ncols << ", result is " << nrows << " x " << ncols << endl;
    return 1;
  }
...
int Mat::LinearSum(double a, double b, const Mat &B) {
  ...
  if (B.nrows != nrows || B.ncols != ncols) {
    ...
    cerr << "  Mat 1 is " << nrows << " x " << ncols << ", Mat 2 is " << 
      B.nrows << " x " << B.ncols << endl;
    return 1;
  }
...
int Mat::LinearSum(double a, const Mat &A, double b, const Mat &B) {
  ...
  if (A.nrows != nrows || A.ncols != ncols || B.nrows != nrows || B.ncols != ncols) {
    cerr << "Mat::LinearSum error, matrix size mismatch\n";
    cerr << "  Mat 1 is " << nrows << " x " << ncols << ",  Mat 2 is " << A.nrows
	 << " x " << A.ncols << ",  Mat 3 is " << B.nrows << " x " << B.ncols << endl;
    return 1;
  }
